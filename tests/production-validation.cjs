...
function validateHTMLContent(html) {
  const checks = {
    ...
    "No obvious errors":
      !html.includes("Error:") && !html.includes("Failed to"),
  };
...
function validateJSBundle(html) {
  ...
  return {
    ...
    "Has worker bundle":
      html.includes("trainingWorker") ||
      scriptMatches.some((s) => s.includes("worker")),
  };
}
...
async function runValidation() {
  ...
  for (const [testSuite, config] of Object.entries(TESTS)) {
    ...
    try {
      ...
      const hasOldError = response.body.includes(
        "isUsingWorker is not defined",
      );
      console.log(
        `  ${hasOldError ? "❌" : "✅"} No "isUsingWorker is not defined" error in HTML`,
      );
      if (hasOldError) allTestsPassed = false;

      // Check for modern JavaScript features
      const hasModernJS =
        response.body.includes('type="module"') ||
        response.body.includes("import") ||
        response.body.includes("export");
      console.log(
        `  ${hasModernJS ? "✅" : "ℹ️ "} Uses modern JavaScript modules`,
      );

      // Check for Web Worker support
      const hasWorkerSupport =
        response.body.includes("trainingWorker") ||
        response.body.includes("Worker");
      console.log(
        `  ${hasWorkerSupport ? "✅" : "⚠️ "} Web Worker support detected`,
      );
    } catch (error) {
  ...
  if (allTestsPassed) {
    ...
    console.log(
      "✅ isUsingWorker component prop destructuring issue appears to be resolved",
    );
  ...
  if (allTestsPassed) {
    ...
    console.log(
      "   3. Test training functionality to ensure no runtime errors",
    );
  } else {
