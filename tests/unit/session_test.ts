...
const exampleSession: AppSessionData = {
  ...
  epochsRun: 15,
  lossHistory: [
    2.3, 1.8, 1.4, 1.1, 0.9, 0.7, 0.6, 0.5, 0.4, 0.4, 0.3, 0.3, 0.2, 0.2, 0.2,
  ],
};
...
class SessionManagerTest {
  static validateSessionData(sessionData: AppSessionData): boolean {
    try {
      ...
      if (
        !Array.isArray(sessionData.layers) ||
        sessionData.layers.length === 0
      ) {
      ...
      for (const dataPoint of sessionData.trainingData) {
        if (
          !Array.isArray(dataPoint.image) ||
          !dataPoint.label ||
          !dataPoint.timestamp
        ) {
          console.error(
            "Training data point missing required fields:",
            dataPoint,
          );
          return false;
        }
  ...
  static calculateSessionSize(sessionData: AppSessionData): {
    ...
    const weightParameters = sessionData.modelWeights
      ? sessionData.modelWeights.reduce(
          (total, weight) => total + weight.data.length,
          0,
        )
      : 0;
  ...
  static runTests(): void {
    ...
    try {
      ...
      console.log(
        "Result: Error FAIL - Serialize/deserialize failed:",
        error,
        "\n",
      );
    }
    ...
    console.log(
      `Result: ${!invalidResult ? "Success PASS" : "Error FAIL"} - Invalid session properly rejected\n`,
    );
