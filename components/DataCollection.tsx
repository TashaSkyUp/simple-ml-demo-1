...
export const DataCollection: React.FC<DataCollectionProps> = ({
  ...
  const { canvasRef, clearCanvas } = useDrawingCanvas({
    onDrawEnd: predictFromCanvas, // This is called on mouse/touch up, and when clearCanvas is called.
    canvasWidth: 280,
    canvasHeight: 280,
    lineWidth: 25,
  });
  ...
  const handleAddData = (label: 0 | 1) => {
    ...
    const originalGrid = imageToGrid(canvasRef.current);

    if (originalGrid.flat().every((pixel) => pixel < 0.01)) {
      alert("Please draw something or generate an image before adding.");
      return;
    }
    ...
    if (augmentFlip && augmentTranslate) {
      const flippedGrid = flipGridHorizontal(originalGrid);
      ...
      for (let i = 0; i < NUM_TRANSLATIONS_PER_BASE; i++) {
        ...
        gridsToSubmit.push(translateGrid(originalGrid, dx, dy));
      }
      for (let i = 0; i < NUM_TRANSLATIONS_PER_BASE; i++) {
        let dx = 0,
          dy = 0;
        ...
        gridsToSubmit.push(translateGrid(flippedGrid, dx, dy));
      }
    } else if (augmentFlip) {
      const flippedGrid = flipGridHorizontal(originalGrid);
      gridsToSubmit.push(flippedGrid);
